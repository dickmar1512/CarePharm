		...
		<table class="table" id="alt">
			...
			<tbody style="min-height: 500px">
				...
				foreach ($dato as $det) {
					...
					?>
					<tr>
						<td><input id="id<?php echo $i; ?>" type="hidden"
								value="<?php echo $det['id']; ?>"><?php echo $det['ctdUnidadItem']; ?></td>
						<td>
							<p><input id="proviejo<?php echo $i; ?>" type="hidden"
									value="<?php echo $det['desItem']; ?>"><?php echo $det['desItem']; ?></p>
							<p id="p<?php echo $i; ?>"><input id="pronuevo<?php echo $i; ?>" type="hidden" value=""></p>
							<p>Cód. Nuevo: <input type="text" id="cod<?php echo $i; ?>"> <a href='javascript:void(0);'
									class='search-product btn btn-primary' data-cod="cod<?php echo $i; ?>"
									data-p="p<?php echo $i; ?>" data-input="pronuevo<?php echo $i; ?>" title='Remove field'><i
										class='glyphicon glyphicon-search'></i></a></p>
						</td>
