...
$NBoletas = count(Boleta2Data::getBoletasSerie('BND1'));
$product = Boleta2Data::getByExtra($_GET["id"]);
  ...
  $query = $mysqli->prepare("SELECT * FROM 1_2_boleta_yaqha");//TABLA
  $query->execute();
  $query->store_result();
  ...
  $SERIE_F = "BND".generaCerosSerie($NUMERO_SERIE_F);
...
<div class="row">
	<div class="col-md-12">
		<h1  style="color: green;"><b><i class="fa fa-file"></i> NOTA DE DEBITO DE BOLETA</b></h1>
		<p style="font-size: 20px">La nota de débito es un documento que se le envía al comprador o cliente para avisarle de que ha aumentado la cantidad de su deuda por algún motivo</p>
		<fieldset class="content-group">
			<legend class="text-bold">Especificaciones: </legend>
				<div class="form-group col-md-4">
				    <label><i class="icon-barcode2 position-left"></i> Serie:</label>
				    <input style="background-color: #FFF;" type="text" name="serie_comprobante" id="serie_comprobante" value="<?php echo $SERIE_F; ?>" class="form-control" readonly="readonly">
			    </div>
			    <div class="form-group col-md-4">
			        <label><i class="icon-file-text2 position-left"></i> Número:</label>
			        <input style="background-color: #FFF;" type="text" name="numero_comprobante" id="numero_comprobante" value="<?php echo $COMPROBANTE_F; ?>" class="form-control" readonly="readonly">
			    </div>
				...
				<div class="content_debito_credito" style="display: block;">
				    <div class="form-group col-md-4">
				        ...
				        <input  style="background-color:yellow;" type="text" name="num_comprobante_modificado" id="num_comprobante_modificado" class="form-control" readonly="readonly" value="<?php echo $product->SERIE."-".$product->COMPROBANTE; ?>">
				    </div>
				...
				<div class="form-group col-md-7">
				    ...
				    <input type="text" autofocus name="motivo" id="motivo" value="" class="form-control" required>
			    </div>
